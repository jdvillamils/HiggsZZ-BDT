...
int other(void){
    ...
    TChain* ochain = new TChain("mini");
    
 
    
    //minor bkgs
    ochain->AddFile(path+"MC/mc_361106.Zee.4lep.root");
    ochain->AddFile(path+"MC/mc_361107.Zmumu.4lep.root");
    ochain->AddFile(path+"MC/mc_410000.ttbar_lep.4lep.root"); 
    ...
    ochain->SetBranchAddress("lep_pt", &Lep_ptb);//, &b_Lep_ptb);
    ochain->SetBranchAddress("trigE", &TrigEb);//, &b_TrigEb);
    ochain->SetBranchAddress("trigM", &TrigMb);//, &b_TrigMb);
    ochain->SetBranchAddress("lep_n", &Lep_nb);//, &b_Lep_nb);
    ochain->SetBranchAddress("lep_phi", &Lep_phib);//, &b_Lep_phib);
    ochain->SetBranchAddress("lep_eta", &Lep_etab);//, &b_Lep_etab);
    ochain->SetBranchAddress("lep_E", &Lep_Eb);//, &b_Lep_Eb);
    ochain->SetBranchAddress("lep_ptcone30", &Lep_ptcone30b);//, &b_Lep_ptcone30b);
    ochain->SetBranchAddress("lep_etcone20", &Lep_etcone20b);//, &b_Lep_etcone20b);
    ochain->SetBranchAddress("lep_type", &Lep_typeb);//, &b_Lep_typeb);
    ochain->SetBranchAddress("lep_charge", &Lep_chargeb);//, &b_Lep_chargeb);
    ochain->SetBranchAddress("lep_trackd0pvunbiased", &lep_trackd0pvunbiasedb);//, &b_lep_trackd0pvunbiasedb);
    ochain->SetBranchAddress("lep_tracksigd0pvunbiased", &lep_tracksigd0pvunbiasedb);//, &b_lep_tracksigd0pvunbiasedb);
    ochain->SetBranchAddress("lep_z0", &Lep_z0b);//, &b_Lep_z0b);
    ochain->SetBranchAddress("jet_n", &jet_nb);//, &b_jet_nb);
    ochain->SetBranchAddress("jet_pt", &jet_ptb);//, &b_jet_ptb);
    ochain->SetBranchAddress("jet_eta", &jet_etab);//, &b_jet_etab);
    
    
    
    //output root file
    
    TFile *target = new TFile("RootFiles/OtherBackground.root","RECREATE");
    
    TTree *other  = new TTree("OtherBackground" , "Other Background");
    ...
    other->Branch("FourLepSystemM", &fourlepsystemb0);

    other->Branch("FourLepSystempt", &fourlepsystemptb0);
    
    other->Branch("InvMassZ1", &invmassz10);

    other->Branch("InvMassZ2", &invmassz20);
    
    other->Branch("FourLepRapidity", &fourrap0);

    other->Branch("FourLepSystemE", &foure0);
...
----------------------------------------
        ...
        int nentriesbo, nbytesbo, bo;

    nentriesbo = (Int_t)ochain->GetEntries();
    for (bo = 0; bo < nentriesbo; bo++)
    {

        nbytesbo = ochain->GetEntry(bo);
        if(TrigEb || TrigMb)
        {
